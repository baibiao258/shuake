...
def build_config_from_args(args):
    """从命令行参数构建配置，优先使用命令行参数，其次读取环境变量(如 GitHub Actions Secrets)

    环境变量:
        CHAOXING_USERNAME  手机号账号
        CHAOXING_PASSWORD  登录密码
        CHAOXING_COURSE_LIST  课程ID列表, 逗号隔开
    这样可以避免账号密码明文出现在命令行参数/脚本中。
    """
    # 将空字符串规范化为 None
    def _norm(value):
        return value if value else None

    username = _norm(args.username) or _norm(os.environ.get("CHAOXING_USERNAME"))
    password = _norm(args.password) or _norm(os.environ.get("CHAOXING_PASSWORD"))
    course_list = _norm(args.list) or _norm(os.environ.get("CHAOXING_COURSE_LIST"))

    common_config = {
        "use_cookies": args.use_cookies,
        "username": username,
        "password": password,
        "course_list": [item.strip() for item in course_list.split(",") if item.strip()] if course_list else None,
